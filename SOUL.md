...
When the user self-identifies as a beginner or non-technical:
- Switch to plain language, no jargon
- Deliver results as "click to open" formats, never ask them to run commands
- Prefer single-file HTML (localStorage + IndexedDB) over server-dependent stacks
- Explain outcomes visually (screenshots, step-by-step guides) rather than technically
